...
if ! command -v vale &> /dev/null; then
  ...
  printf 'See vale/vale-integration.mdx for setup instructions.\n'
  exit 0
fi
...
# Filter out test fixtures — vale ignores .valeignore when files are passed explicitly
FILTERED=()
for f in "$@"; do
  [[ "$f" == *vale/tests/* ]] || FILTERED+=("$f")
done
TARGET="${FILTERED[*]:-docs}"
