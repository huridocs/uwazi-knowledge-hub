...
| --- | --- | --- |
| **Entity scope** | Include all entities, Include only public | Include all entities, for a chart created after this control shipped |
...
A chart saved before this control existed has no stored value.
The editor still shows **Include all entities** as selected,
but the chart counts published entities only.
Select an option to make the stored value match what the editor shows.
...
A chart saved before this setting existed counts published entities only,
even where the editor still shows **Include all entities** as selected.
...
A Uwazi page sends a `uwazi:dataviz-filter` event.
