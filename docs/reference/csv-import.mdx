...
| Multilingual property missing any installed language  | Import fails |
...
A property that carries any language suffix needs a column
for every language installed in the instance.
Uwazi names the missing codes in the failure message.

### Language suffix support by property type

| Property type        | Language suffix |
| -------------------- | --------------- |
| Title                | Supported       |
| Text                 | Supported       |
| Markdown             | Supported       |
...
| Date                 | Rejected        |
| Date range           | Rejected        |
| Multiple dates       | Rejected        |
| Multiple date ranges | Rejected        |
| Geolocation          | Rejected        |
| Relationship         | Rejected        |
| Generated ID         | Rejected        |

Select and multi-select accept the suffixed headers,
but the value itself doesn't vary by language.
Uwazi reads the default-language column
and applies the matched thesaurus value to every language.
...
| -------------------- | -------------------------------------------------------------------------------- | ----------------------------------------------- |
| Title                | Plain text                                                                       | An empty cell fails the row                     |
...
| Value required         | `VALUE_REQUIRED`           | A title cell is empty in one of the languages              |
...
A **Value required** error names the exact column,
such as `title` or `title__fr`.
Uwazi checks the title once per language,
so an empty translation fails the row even
when the default-language title holds a value.
